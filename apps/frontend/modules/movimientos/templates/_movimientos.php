<?php if ($movimientos->count()): foreach ($movimientos->getResults() as $mo): echo PHP_EOL; ?>
                            <tr>
                                ...
                                <td>
                                    <a href="#modal<?php echo $mo->getId() ?>" role="button" data-toggle="modal" data-placement="right" title="<?php echo $mo->getMoOficina() ?>"<?php if (!is_null($mo->getMoMiniDetalleJson())) echo ' style="cursor: pointer; color: #F06C00"' ?>><?php echo $mo->getMoConcepto() ?></a>
